...
class WorkingCapitalControllerSecurityTest {
  ...
  void shouldReturn401WhenUnauthenticated() throws Exception {
    mockMvc.perform(get("/api/v1/analytics/working-capital")).andExpect(status().isUnauthorized());
  }
  ...
  void shouldReturn403WhenMissingFinanceReadPermission() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/analytics/working-capital")).andExpect(status().isForbidden());
  }
  ...
  void shouldReturn200WhenHasFinanceReadPermission() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/analytics/working-capital")).andExpect(status().isOk());
  }
}
