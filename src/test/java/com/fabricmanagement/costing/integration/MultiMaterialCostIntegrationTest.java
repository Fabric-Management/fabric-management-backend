...
class MultiMaterialCostIntegrationTest
  ...
  @Test
  @DisplayName(
      "Per-material pricing with foreign currency translates accurately to reporting currency")
  void consumesMixedMaterials_withCurrencyConversion_calculatesCorrectly() {
    ...
    assertThat(result.getLines()).hasSize(3); // 2 Raw Material paths + 1 Labor
    ...
    CostCalculationLine fiberBLine = findLine(result, "RAW_MATERIAL", fiberBId);
    ...
    CostCalculationLine fiberALine = findLine(result, "RAW_MATERIAL", fiberAId);
  ...
  private CostCalculationLine findLine(
      CostCalculation calc, String expectedCode, UUID expectedMaterial) {
    return calc.getLines().stream()
        .filter(l -> l.getCostItemCode().equals(expectedCode))
        .filter(l -> expectedMaterial == null || expectedMaterial.equals(l.getMaterialId()))
        .findFirst()
        .orElseThrow();
  }
}
