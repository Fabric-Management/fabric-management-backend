...
class ReceivablesInsightControllerSecurityTest {
  ...
  @Test
  @WithMockUser
  void whenUserHasFinanceRead_thenSummaryAndCustomersReturn200() throws Exception {
    when(authEvaluator.can(any(Authentication.class), eq("finance"), eq("read"))).thenReturn(true);
    when(receivablesInsightService.getCustomers(any())).thenReturn(new PageImpl<>(List.of()));

    mockMvc.perform(get("/api/v1/finance/receivables/summary")).andExpect(status().isOk());
    mockMvc.perform(get("/api/v1/finance/receivables/customers")).andExpect(status().isOk());
  }
  ...
  void whenUserLacksFinanceRead_thenEndpointsReturn403() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/finance/receivables/summary")).andExpect(status().isForbidden());
    mockMvc.perform(get("/api/v1/finance/receivables/customers")).andExpect(status().isForbidden());
  }
}
