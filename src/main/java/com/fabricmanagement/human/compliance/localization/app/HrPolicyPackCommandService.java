...
public class HrPolicyPackCommandService {
  ...
  public HrPolicyPackResponse createDraft(CreateHrPolicyPackRequest request) {
    ...
    Optional<HrPolicyPack> existingDraft =
        policyPackService.findDraftByPackCode(tenantId, packCode);
    ...
    int nextVersion =
        policyPackService
            .findLatestByPackCode(tenantId, packCode)
  ...
  public HrPolicyPackResponse updateDraft(String packCode, UpdateHrPolicyPackRequest request) {
    ...
    HrPolicyPack draft =
        policyPackService
            ...
            .orElseThrow(
                () -> new IllegalStateException("Draft not found for packCode=" + packCode));
    ...
    draft.updateHierarchy(
        parentPackId,
        normalizeCode(request.parentPackCode()),
        normalizeRegion(request.regionCode()),
  ...
  public ValidateHrPolicyPackResponse validatePayload(String payload) {
    try {
      ...
      return new ValidateHrPolicyPackResponse(
          false, null, "Invalid JSON payload: " + ex.getMessage());
    }
  }
  ...
  public HrPolicyPackResponse publish(String packCode, PublishHrPolicyPackRequest request) {
    ...
    policyPackService
        .findActivePack(tenantId, draft.getCountryCode())
    ...
    var affectedMappings =
        countryPackMappingService.updateMappingsForPack(
            tenantId, saved.getPackCode(), saved.getId());
  ...
  public HrPolicyPackResponse retire(String packCode, RetireHrPolicyPackRequest request) {
    ...
    HrPolicyPack pack =
        policyPackService
            ...
            .orElseThrow(
                () -> new IllegalStateException("Active pack not found for packCode=" + packCode));
  ...
  private List<HrRuleVersion> mapRuleVersions(
      List<RuleVersionRequest> requests, String packChecksum) {
    if (CollectionUtils.isEmpty(requests)) {
      return List.of();
    }
  ...
  private UUID resolveParentPackId(UUID tenantId, String parentPackCode) {
    ...
    return policyPackRepository
        .findFirstByTenantIdAndPackCodeOrderByPackVersionDesc(
            tenantId, parentPackCode.toUpperCase())
        .map(HrPolicyPack::getId)
        .orElseThrow(
            () -> new IllegalArgumentException("Parent pack not found: " + parentPackCode));
  }

  private void assertNoInheritanceCycle(
      UUID tenantId, String childPackCode, String parentPackCode) {
    if (parentPackCode == null || parentPackCode.isBlank()) {
      return;
    }
    ...
    while (currentParent != null) {
      if (normalizedChild.equals(currentParent)) {
        throw new IllegalArgumentException(
            "Inheritance cycle detected for packCode=" + childPackCode);
      }
      String lookupCode = currentParent;
      HrPolicyPack parentPack =
          policyPackRepository
              .findFirstByTenantIdAndPackCodeOrderByPackVersionDesc(tenantId, lookupCode)
              .orElseThrow(
                  () -> new IllegalArgumentException("Parent pack not found: " + lookupCode));
      currentParent = normalizeCode(parentPack.getParentPackCode());
      guard++;
      if (guard > 20) {
        throw new IllegalArgumentException(
            "Inheritance chain too deep or cyclical for packCode=" + childPackCode);
      }
    }
  }
  ...
  private List<String> computeLineage(HrPolicyPack pack) {
    ...
    while (current != null && guard < 20) {
      ...
      HrPolicyPack parent =
          policyPackRepository
              .findFirstByTenantIdAndPackCodeOrderByPackVersionDesc(tenantId, current)
              .orElse(null);
