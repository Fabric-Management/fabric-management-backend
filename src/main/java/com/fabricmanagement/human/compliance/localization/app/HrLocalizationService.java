...
public class HrLocalizationService {
    ...
    public Optional<HrPolicyPack> resolveActivePack() {
        ...
        return resolveResolvedPack()
            .flatMap(resolved -> policyPackService.findActiveByPackCode(context.tenantId(), resolved.packCode()))
            .or(() -> policyPackService.findActivePack(context.tenantId(), context.tenantCountryCode()))
            .or(() -> policyPackService.findActivePack(context.tenantId(), HrLocalizationConstants.GLOBAL_COUNTRY_CODE));
    }
