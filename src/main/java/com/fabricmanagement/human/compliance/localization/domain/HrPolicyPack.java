...
@Entity
@Table(
    name = "human_hr_policy_pack",
    schema = "human",
    indexes = {
      @Index(
          name = "idx_hr_policy_pack_tenant_country",
          columnList = "tenant_id,country_code,status"),
      @Index(
          name = "idx_hr_policy_pack_code",
          columnList = "tenant_id,pack_code,pack_version",
          unique = true)
    })
@Getter
@Setter
...
public class HrPolicyPack extends BaseEntity {
  ...
  public void updateHierarchy(
      UUID parentPackId, String parentPackCode, String regionCode, HrInheritanceMode mode) {
    this.parentPackId = parentPackId;
    this.parentPackCode = parentPackCode;
    this.regionCode = regionCode;
    if (mode != null) {
