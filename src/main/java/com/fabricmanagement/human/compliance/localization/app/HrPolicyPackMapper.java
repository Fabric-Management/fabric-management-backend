  ...
  public static HrPolicyPackResponse toResponse(HrPolicyPack pack) {
    return new HrPolicyPackResponse(
        ...
        pack.getPolicyBindings().stream()
            ...
            .map(
                binding ->
                    new PolicyBindingDto(
                        binding.getPolicyInterface(),
                        binding.getStrategyBean(),
                        binding.getConfigReference()))
            .toList(),
        pack.getRuleVersions().stream()
            ...
            .map(
                version ->
                    new RuleVersionDto(
                        version.getRuleType(), version.getPayloadHash(), version.getPayload()))
            .toList());
  }

  public static List<HrPolicyPackResponse> toResponseList(List<HrPolicyPack> packs) {
    return packs.stream().map(HrPolicyPackMapper::toResponse).toList();
  }
