...
public class UsPayrollStrategy implements PayrollStrategy {
    ...
    public PayrollResult execute(PayrollContext context) {
        ...
        List<Map<String, Object>> federalBrackets = getList(payrollConfig, "payroll.federalTax.brackets");
    ...
    protected BigDecimal getEmployeeGross(PayrollContext context, UUID employeeId) {
        Object value = context.policyParameters().getOrDefault("employeeGross:" + employeeId, BigDecimal.valueOf(5000));
    ...
    protected BigDecimal applyProgressiveTax(BigDecimal gross, List<Map<String, Object>> brackets) {
        ...
        if (remaining > 0) {
            double topRate = ((Number) brackets.get(brackets.size() - 1).getOrDefault("rate", 0.22)).doubleValue();
