...
public class EuPayrollStrategy implements PayrollStrategy {
  ...
  protected BigDecimal getEmployeeGross(PayrollContext context, UUID employeeId) {
    Object value =
        context
            .policyParameters()
            .getOrDefault("employeeGross:" + employeeId, BigDecimal.valueOf(3000));
  ...
  protected BigDecimal applyProgressiveTax(BigDecimal gross, List<Map<String, Object>> brackets) {
    ...
    if (remaining > 0) {
      double topRate =
          ((Number) brackets.get(brackets.size() - 1).getOrDefault("rate", 0.2)).doubleValue();
      tax = tax.add(BigDecimal.valueOf(remaining * topRate));
    }
    return tax;
  }

  protected BigDecimal applySocialContributions(
      BigDecimal gross, Map<String, Object> contributions) {
