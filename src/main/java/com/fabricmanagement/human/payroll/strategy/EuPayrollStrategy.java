...
public class EuPayrollStrategy implements PayrollStrategy {

    @Override
    public boolean supports(String countryCode) {
        return countryCode != null && switch (countryCode.toUpperCase(Locale.ROOT)) {
            case "EU", "FR", "DE", "IT", "ES", "UK" -> true;
            default -> false;
        };
    }
    ...
    public PayrollResult execute(PayrollContext context) {
        ...
        List<Map<String, Object>> taxBrackets = getList(payrollConfig, "payroll.taxBrackets");
        ...
        for (UUID employeeId : context.employeeIds()) {
            ...
            BigDecimal tax = applyProgressiveTax(gross, taxBrackets);
            BigDecimal social = applySocialContributions(gross, contributions);
            BigDecimal net = gross.subtract(tax).subtract(social);
    ...
    protected BigDecimal getEmployeeGross(PayrollContext context, UUID employeeId) {
        Object value = context.policyParameters().getOrDefault("employeeGross:" + employeeId, BigDecimal.valueOf(3000));
    ...
    protected BigDecimal applyProgressiveTax(BigDecimal gross, List<Map<String, Object>> brackets) {
        ...
        if (brackets == null || brackets.isEmpty()) {
            return gross.multiply(BigDecimal.valueOf(0.2));
        }
        double remaining = gross.doubleValue();
        double lastThreshold = 0;
        for (Map<String, Object> bracket : brackets) {
            double threshold = ((Number) bracket.getOrDefault("threshold", Double.MAX_VALUE)).doubleValue();
            double rate = ((Number) bracket.getOrDefault("rate", 0.2)).doubleValue();
        ...
        if (remaining > 0) {
            double topRate = ((Number) brackets.get(brackets.size() - 1).getOrDefault("rate", 0.2)).doubleValue();
            tax = tax.add(BigDecimal.valueOf(remaining * topRate));
        }
        return tax;
    }

    protected BigDecimal applySocialContributions(BigDecimal gross, Map<String, Object> contributions) {
        if (contributions == null || contributions.isEmpty()) {
            return gross.multiply(BigDecimal.valueOf(0.1));
        }
        double rateSum = contributions.values().stream()
            .filter(Number.class::isInstance)
            .mapToDouble(value -> ((Number) value).doubleValue())
            .sum();
        return gross.multiply(BigDecimal.valueOf(rateSum));
    }
