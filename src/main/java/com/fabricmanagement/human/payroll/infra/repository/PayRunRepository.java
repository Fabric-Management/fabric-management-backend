...
public interface PayRunRepository extends JpaRepository<PayRun, UUID> {
    ...
    @Query("""
        ...
        """)
    List<PayRun> findByStatus(@Param("tenantId") UUID tenantId,
                              @Param("status") PayRunStatus status);

    Optional<PayRun> findFirstByTenantIdAndPayPeriodOrderByRunNumberDesc(UUID tenantId, PayPeriod payPeriod);
