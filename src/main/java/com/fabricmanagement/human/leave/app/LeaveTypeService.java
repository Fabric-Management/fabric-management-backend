...
public class LeaveTypeService {
    ...
    public LeaveType getActiveByCode(UUID tenantId, String leaveTypeCode) {
        return leaveTypeRepository.findActiveByCode(tenantId, normalize(leaveTypeCode))
            .orElseThrow(() -> new IllegalArgumentException("Leave type not found or inactive: " + leaveTypeCode));
    }

    public List<LeaveType> findActiveForCountry(UUID tenantId, String countryCode) {
        return leaveTypeRepository.findActiveForCountry(tenantId, countryCode != null ? countryCode.toUpperCase(Locale.ROOT) : null);
    }
