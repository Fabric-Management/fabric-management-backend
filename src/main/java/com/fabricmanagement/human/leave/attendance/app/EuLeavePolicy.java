...
public class EuLeavePolicy implements LeavePolicy {
  ...
  public LeaveAccrualResult calculateAccrual(LeavePolicyRequest request) {
    double baseAnnualLeave =
        getDouble(request.policyAttributes(), "leave.default.annualLeaveDays", 20d);
    double carryOverDays =
        getDouble(request.policyAttributes(), "leave.default.carryOverDays", 10d);
    ...
    if (startDate != null) {
      long daysEmployeed =
          ChronoUnit.DAYS.between(
              startDate, request.asOfDate().atZone(clock.getZone()).toLocalDate());
      int probationDays =
          (int) getDouble(request.policyAttributes(), "leave.default.probationDays", 0);
  ...
  protected double getDouble(
      java.util.Map<String, Object> attributes, String path, double defaultValue) {
