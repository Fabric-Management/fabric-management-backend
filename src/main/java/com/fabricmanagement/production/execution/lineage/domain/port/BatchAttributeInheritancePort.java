 ...
 * material-type and business-rule specific (e.g. Yarn module implements for FIBER→YARN).
 *
...
public interface BatchAttributeInheritancePort {
   ...
   *   <li>{@code sourceType} — material type of the consumed parent batch(es) (e.g. FIBER, YARN).
   *       Together with {@code targetType} determines which inheritance schema applies (e.g.
   *       FIBER→YARN, YARN→FABRIC). Never null.
   *   <li>{@code targetType} — material type of the batch being produced (e.g. YARN, FABRIC).
   *       Drives which inheritance schema/rules apply. Never null.
   * </ul>
   *
   ...
   * @param sourceType material type of the consumed parent batch(es); never null
   * @param targetType material type of the batch being produced; never null
