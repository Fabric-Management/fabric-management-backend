...
public class FiberBatchService {
  ...
  public List<FiberBatchDto> getAll() {
    ...
    return fiberBatchRepository.findByTenantIdAndIsActiveTrue(tenantId).stream()
  ...
  public List<FiberBatchDto> getByFiberId(UUID fiberId) {
    ...
    return fiberBatchRepository.findByTenantIdAndFiberIdAndIsActiveTrue(tenantId, fiberId).stream()
        .map(FiberBatchDto::from)
        .toList();
  }
  ...
  public FiberBatchDto reserve(UUID batchId, BigDecimal quantity) {
    ...
    log.info(
        "Reserved batch: id={}, reservedQty={}, availableQty={}",
        saved.getId(),
  ...
  public FiberBatchDto release(UUID batchId, BigDecimal quantity) {
    ...
    FiberBatch saved = fiberBatchRepository.save(batch);
    log.info(
        "Released batch: id={}, reservedQty={}, availableQty={}",
        saved.getId(),
        saved.getReservedQuantity(),
        saved.getAvailableQuantity());
