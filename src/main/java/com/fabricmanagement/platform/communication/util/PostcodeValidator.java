 ...
 *
 * <p>Validates postcode formats before making Google Maps API calls to reduce unnecessary requests.
 */
@Component
@Slf4j
public class PostcodeValidator {
  ...
  private static final Pattern UK_POSTCODE =
      Pattern.compile("^[A-Z]{1,2}[0-9][A-Z0-9]?\\s?[0-9][A-Z]{2}$", Pattern.CASE_INSENSITIVE);
  ...
  private static final Pattern CA_POSTCODE =
      Pattern.compile("^[A-Z][0-9][A-Z]\\s?[0-9][A-Z][0-9]$", Pattern.CASE_INSENSITIVE);
  ...
  private static final Pattern NL_POSTCODE =
      Pattern.compile("^[0-9]{4}\\s?[A-Z]{2}$", Pattern.CASE_INSENSITIVE);
  ...
  public boolean isValidFormat(String postcode, String countryCode) {
    ...
    return switch (upperCountry) {
      ...
      default -> {
        ...
        log.debug(
            "Unknown country code for postcode validation: {}. Using minimum length check (4 chars)",
            countryCode);
  ...
  public int getMinimumLength(String countryCode) {
    ...
    return switch (upperCountry) {
      case "GB", "UK" ->
          5; // MK1 1QB = 7, but allow partial (MK1 = 3, but we require 5 for validation)
