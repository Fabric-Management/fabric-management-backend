 ...
 *
 * <p>Converts audio files (mp3, wav, etc.) to text using OpenAI's Whisper model.
 */
@Service
@Slf4j
public class WhisperService {

  private static final String OPENAI_WHISPER_API_URL =
  ...
  public String transcribe(MultipartFile audioFile) {
    ...
    log.info(
        "Whisper transcription request: tenantId={}, userId={}, fileName={}, fileSize={} bytes",
        tenantId,
        userId,
    ...
    try {
      ...
      ResponseEntity<String> response =
          restTemplate.exchange(
              OPENAI_WHISPER_API_URL, HttpMethod.POST, requestEntity, String.class);
      ...
      log.info(
          "Whisper transcription successful: tenantId={}, userId={}, textLength={}",
          tenantId,
          userId,
      ...
      log.error(
          "OpenAI API error: status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString());
  ...
  private void validateFile(MultipartFile file) {
    ...
    if (file.getSize() > MAX_FILE_SIZE) {
      throw new IllegalArgumentException(
          String.format(
              "File size exceeds maximum limit: %d bytes (max: %d bytes)",
    ...
    if (contentType == null || !isValidAudioType(contentType)) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid file type: %s. Supported types: audio/mpeg, audio/wav, audio/webm, audio/m4a, audio/x-m4a, audio/mp3",
              contentType));
    }
  }
   ...
   *
   * <ul>
   *   <li>"31 gabardin" → "30/1 gabardin"
   *   <li>"41 pamuk" → "40/1 pamuk"
   *   <li>"21 keten" → "20/1 keten"
   * </ul>
   *
