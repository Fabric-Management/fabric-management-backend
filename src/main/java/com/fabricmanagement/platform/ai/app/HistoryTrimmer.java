 ...
 *
 * <p>Smart trimming: Keeps system message, recent messages, and summarizes old ones.
 *
 * <p>MANIFESTO: KISS - Simple token estimation, no complex ML
 */
@Component
@Slf4j
public class HistoryTrimmer {
  ...
  // System message: ~500 tokens, User message: ~50 tokens, Assistant: ~100 tokens, Tool result:
  // ~200 tokens
   ...
   *
   * <ul>
   *   <li>Keep system message (always)
   *   <li>Keep recent messages (last N)
   *   <li>Remove or summarize old messages if needed
   * </ul>
   *
  ...
  public List<Map<String, Object>> trim(List<Map<String, Object>> history) {
    ...
    Map<String, Object> systemMsg =
        history.stream().filter(msg -> "system".equals(msg.get("role"))).findFirst().orElse(null);
    ...
    log.info(
        "Trimmed history: {} messages -> {} messages, ~{} tokens",
        history.size(),
        trimmed.size(),
  ...
  private List<Map<String, Object>> aggressiveTrim(
      List<Map<String, Object>> messages, int maxTokens) {
    List<Map<String, Object>> result = new ArrayList<>();

    // Keep system message
    Map<String, Object> systemMsg =
        messages.stream().filter(msg -> "system".equals(msg.get("role"))).findFirst().orElse(null);
