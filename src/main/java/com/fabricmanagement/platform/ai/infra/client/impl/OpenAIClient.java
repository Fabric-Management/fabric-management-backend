package com.fabricmanagement.platform.ai.infra.client.impl;

import com.fabricmanagement.platform.ai.config.AIProperties;
import com.fabricmanagement.platform.ai.dto.ChatRequest;
import com.fabricmanagement.platform.ai.dto.ChatResponse;
import com.fabricmanagement.platform.ai.infra.client.LLMClient;
 ...
 *
 * <p>Implements LLMClient interface for OpenAI API integration.
 *
 * <p>Bean is registered via AIConfig based on application.ai.provider configuration.
 */
@Slf4j
public class OpenAIClient implements LLMClient {
  ...
  public ChatResponse chat(ChatRequest request) {
    ...
    if (aiProperties.getApiKey() == null || aiProperties.getApiKey().isBlank()) {
      throw new IllegalStateException(
          "AI API key is not configured. Please set OPENAI_API_KEY in .env file or as environment variable.");
    }

    log.debug(
        "Sending chat request to OpenAI: model={}, messages={}",
      ...
      try {
        ...
        ResponseEntity<OpenAIResponse> response =
            restTemplate.exchange(
                OPENAI_API_URL, HttpMethod.POST, httpEntity, OpenAIResponse.class);
        ...
        if (attempt < MAX_RETRIES) {
          ...
          log.warn(
              "OpenAI API returned {} (attempt {}/{}) - retrying in {}ms: {}",
              statusCode,
              attempt,
              MAX_RETRIES,
              delayMs,
          ...
          log.error(
              "OpenAI API failed after {} attempts with {}: {}",
              MAX_RETRIES,
              statusCode,
              e.getMessage());
        }
        ...
        throw new RuntimeException(
            String.format(
                "OpenAI API client error (%d): %s. Please check your API key and request format.",
        ...
        if (attempt < MAX_RETRIES) {
          long delayMs = INITIAL_RETRY_DELAY_MS * (long) Math.pow(2, attempt - 1);
          log.warn(
              "OpenAI API network error (attempt {}/{}) - retrying in {}ms: {}",
              attempt,
              MAX_RETRIES,
              delayMs,
              e.getMessage());
    ...
    throw new RuntimeException(
        String.format(
            "Failed to get response from OpenAI after %d attempts. Last error: %s",
  ...
  private Map<String, Object> buildRequestBody(ChatRequest request) {
    ...
    body.put(
        "max_tokens",
        request.getMaxTokens() != null ? request.getMaxTokens() : aiProperties.getMaxTokens());
  ...
  private ChatResponse mapToChatResponse(OpenAIResponse openAIResponse) {
    ...
    ChatResponse.TokenUsage tokenUsage =
        ChatResponse.TokenUsage.builder()
            .promptTokens(
                openAIResponse.getUsage() != null ? openAIResponse.getUsage().getPromptTokens() : 0)
            .completionTokens(
                openAIResponse.getUsage() != null
                    ? openAIResponse.getUsage().getCompletionTokens()
                    : 0)
            .totalTokens(
                openAIResponse.getUsage() != null ? openAIResponse.getUsage().getTotalTokens() : 0)
            .build();
