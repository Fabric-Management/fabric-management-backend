...
import com.fabricmanagement.platform.user.domain.UserDepartmentId;
 ...
 *
 * <p>Manages Many-to-Many relationships between User and Department.
 */
@Repository
public interface UserDepartmentRepository extends JpaRepository<UserDepartment, UserDepartmentId> {
  ...
  @Query(
      "SELECT ud.departmentId FROM UserDepartment ud "
          + "JOIN ud.user u "
          + "WHERE u.tenantId = :tenantId AND ud.userId = :userId")
  List<UUID> findDepartmentIdsByTenantIdAndUserId(
      @Param("tenantId") UUID tenantId, @Param("userId") UUID userId);
  ...
  List<UserDepartment> findByTenantIdAndDepartmentId(
      @Param("tenantId") UUID tenantId, @Param("departmentId") UUID departmentId);
