 ...
 *
 * <p>Defines available OS packages that tenants can subscribe to. Each OS provides specific modules
 * and features.
 *
 * <h2>Available OS Examples:</h2>
 *
 * <ul>
 *   <li><b>YarnOS:</b> Yarn production management (Starter, Professional, Enterprise)
 *   <li><b>LoomOS:</b> Weaving/Loom operations (Starter, Professional, Enterprise)
 *   <li><b>AnalyticsOS:</b> BI & Reporting (Standard, Advanced, Enterprise)
 *   <li><b>IntelligenceOS:</b> AI/ML (Professional, Enterprise)
 *   <li><b>FabricOS:</b> Base platform (Base tier, included for all)
 * </ul>
 *
 ...
 * <h2>Pricing Tiers:</h2>
 *
 * <p>Each OS can have multiple pricing tiers with different features. Tier names are defined in
 * {@link PricingTierValidator}.
 */
@Entity
@Table(
    name = "common_os_definition",
    schema = "common_company",
    indexes = {@Index(name = "idx_os_code", columnList = "os_code", unique = true)})
@Getter
@Setter
@Builder
@NoArgsConstructor
...
public class OSDefinition extends BaseEntity {
   ...
   *
   * <ul>
   *   <li>YarnOS: ["Starter", "Professional", "Enterprise"]
   ...
   *
   * <p>Used when creating new subscriptions without explicit tier selection.
   */
  ...
  private void validate() {
    ...
    if (availableTiers == null || availableTiers.isEmpty()) {
      this.availableTiers = new ArrayList<>(PricingTierValidator.getValidTiers(osCode));
    }
    ...
    if (defaultTier != null && !availableTiers.contains(defaultTier)) {
      throw new IllegalStateException(
          String.format(
              "Default tier '%s' must be in available tiers: %s", defaultTier, availableTiers));
    }
  }
