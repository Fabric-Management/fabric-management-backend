 ...
 *
 * <p>Handles multi-word queries, synonyms, and partial matches.
 *
 * <p>Uses related entities (Fiber, Yarn, Fabric) for name-based matching.
 */
@Component
...
public class MaterialMatcher {
  ...
  /**
   * Technical textile term patterns (regex) to preserve during matching. These patterns should not
   * be normalized or split into words.
   */
  private static final java.util.List<java.util.regex.Pattern> TECHNICAL_TERM_PATTERNS =
      java.util.List.of(
          ...
          java.util.regex.Pattern.compile(
              "\\d+\\s*gsm", java.util.regex.Pattern.CASE_INSENSITIVE), // 190 GSM
          java.util.regex.Pattern.compile("%\\d+"), // %100
          java.util.regex.Pattern.compile(
              "\\d+['']?a\\s*bir", java.util.regex.Pattern.CASE_INSENSITIVE), // 30'a bir
          java.util.regex.Pattern.compile(
              "\\d+['']?ye\\s*iki", java.util.regex.Pattern.CASE_INSENSITIVE) // 40'ye iki
          );
  ...
  public List<MaterialDto> findMatches(String query, List<MaterialDto> materials) {
    ...
    for (MaterialDto material : materials) {
      ...
      if (score > 0) {
        log.debug(
            "Material match: uid={}, name={}, score={}, query={}",
            materialUid,
            materialName,
            score,
            normalizedQuery);
    ...
    return matches.stream().map(m -> m.material).collect(Collectors.toList());
  }

  /**
   * Normalize query while preserving technical textile terms. Converts to lowercase but keeps
   * technical patterns intact.
   */
  private String normalizeQueryPreservingTechnicalTerms(String query) {
    ...
    boolean hasTechnicalTerms =
        TECHNICAL_TERM_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(query).find());
    ...
    if (techMatcher != null) {
      ...
      String techTerm =
          query.substring(techStart, techEnd); // Preserve original case and formatting
  ...
  /**
   * Expand query with synonyms and word variations. Technical terms are preserved and not expanded.
   */
  private Set<String> expandQuery(String query) {
    ...
    java.util.function.Predicate<String> isTechnicalTerm =
        word -> TECHNICAL_TERM_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(word).find());
  ...
  /**
   * Get material name from related entity (Fiber, Yarn, Fabric). Returns empty string if no related
   * entity found.
   */
  ...
  /**
   * Calculate match score for a material. Higher score = better match
   *
  ...
  private int calculateMatchScore(
      String query, Set<String> searchTerms, String materialUid, String materialName) {
    int score = 0;
  ...
  /**
   * Extract base name from material UID. Examples: - "MAT-001-COTTON" → "cotton" - "COTTON" →
   * "cotton" - "cotton-fiber" → "cotton"
   */
