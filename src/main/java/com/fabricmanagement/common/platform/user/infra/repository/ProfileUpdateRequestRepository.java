...
public interface ProfileUpdateRequestRepository extends JpaRepository<ProfileUpdateRequest, UUID> {
     ...
     */
    List<ProfileUpdateRequest> findByTenantIdAndUserIdOrderByCreatedAtDesc(UUID tenantId, UUID userId);

    /**
     * Find all pending requests in a tenant.
     */
    List<ProfileUpdateRequest> findByTenantIdAndStatusOrderByCreatedAtAsc(UUID tenantId, ProfileUpdateRequestStatus status);
