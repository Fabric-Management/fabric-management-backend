 ...
 *
 * <p>Handles dynamic role management (database-driven, not enums).
 *
 * <p>Key responsibilities:
 *
 * <ul>
 *   <li>Role CRUD with tenant isolation
 *   <li>Role code uniqueness validation
 *   <li>Role assignment to users
 * </ul>
 */
@Service
...
public class RoleService {
  ...
  public List<Role> findAll() {
    ...
    // Tenant-specific roles are not returned (they can be created but are not shown in standard
    // lists)
    ...
    log.debug(
        "Found {} platform roles (from SYSTEM_TENANT_ID) for current tenant context: tenantId={}",
        systemRoles.size(),
        tenantId);
    return systemRoles;
  }
  ...
  public Role create(String roleName, String roleCode, String description) {
    ...
    log.info(
        "Role created: id={}, uid={}, code={}", saved.getId(), saved.getUid(), saved.getRoleCode());
