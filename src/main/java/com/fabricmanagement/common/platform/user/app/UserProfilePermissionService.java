 ...
 *   <li>Self-update: DENIED for all categories (security)
 * </ul>
 */
@Service
...
public class UserProfilePermissionService {
  ...
  public boolean canUpdateWorkProfile(UUID requesterId, UUID targetUserId) {
    ...
    if ("DEPT_MANAGER".equals(roleCode) || "MANAGER".equals(roleCode)) {
      ...
      if (sameDepartment) {
        log.debug(
            "Department Manager allowed (same dept): requesterId={}, targetUserId={}",
            requesterId,
            targetUserId);
        return true;
      }
      log.debug(
          "Department Manager denied (different dept): requesterId={}, targetUserId={}",
          requesterId,
          targetUserId);
      return false;
    }
    ...
    log.debug(
        "No permission: requesterId={}, roleCode={}, targetUserId={}",
        requesterId,
        roleCode,
        targetUserId);
    return false;
  ...
  public boolean canUpdatePersonalProfile(UUID requesterId, UUID targetUserId) {
    ...
    if (allowed) {
      log.debug(
          "Personal profile update allowed: requesterId={}, roleCode={}, targetUserId={}",
          requesterId,
          roleCode,
          targetUserId);
    } else {
      log.debug(
          "Personal profile update denied: requesterId={}, roleCode={}, targetUserId={}",
          requesterId,
          roleCode,
          targetUserId);
    }
  ...
  /** Check if requester can view target user's profile. Users can always view their own profile. */
  @Transactional(readOnly = true)
