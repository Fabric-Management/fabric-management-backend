 ...
 * TenantContext.getCurrentTenantId()} as the first argument — never query without it.
 */
@Repository
public interface UserNavPreferencesRepository extends JpaRepository<UserNavPreferences, UUID> {
   ...
   * com.fabricmanagement.common.infrastructure.persistence.TenantContext#getCurrentTenantId()} for
   * {@code tenantId} to enforce tenant isolation.
   *
   * @param tenantId must be from TenantContext.getCurrentTenantId()
