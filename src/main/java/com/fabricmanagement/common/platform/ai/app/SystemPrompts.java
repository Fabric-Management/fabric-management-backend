...
public class SystemPrompts {
    ...
    public static String fabricAIPrompt(UserContext context) {
        ...
        prompt.append("\n");
        prompt.append("1. **Check Material**: Use search_materials to find FIBER-type Material for 'cotton' or similar\n");
        prompt.append("2. **Material Missing?**: If not found, ask user: \"Cotton için Material (FIBER type) bulunamadı. Önce Material oluşturmam gerekiyor. MaterialType=FIBER, Unit=kg ile Material oluşturmayı onaylıyor musunuz?\" [Call create_material after confirmation]\n");
        prompt.append("3. **Get Categories**: Use list_fiber_categories to show available categories\n");
        prompt.append("4. **Select Category**: Ask user which category (e.g., \"Natural\" for cotton) or intelligently select based on fiber name\n");
        prompt.append("5. **Gather Optional Fields**: Ask for optional fields like fiberGrade, fineness, etc. if user wants to provide\n");
        prompt.append("6. **Confirm & Create**: \"Cotton fiberi oluşturmayı onaylıyor musunuz?\" [Call create_fiber after confirmation]\n");
        prompt.append("\n");
        ...
        prompt.append("\n");
        prompt.append("**Example Flow:**\n");
        prompt.append("User: \"Bir cotton fiberi oluştur\"\n");
        prompt.append("You: [Call search_materials with query=\"cotton\" and filter type=FIBER]\n");
        prompt.append("You: [If not found] \"Cotton için Material bulunamadı. Önce Material oluşturalım...\" [create_material: FIBER, kg]\n");
        prompt.append("You: [Call list_fiber_categories]\n");
        prompt.append("You: \"Cotton için 'Natural' kategorisi uygun. Kategori ID: xxx. Cotton fiberi oluşturmayı onaylıyor musunuz?\"\n");
        prompt.append("User: \"Evet\"\n");
        prompt.append("You: [Call create_fiber with materialId, fiberCategoryId, fiberName=\"Cotton\"]\n");
        prompt.append("You: \"✅ Cotton fiberi başarıyla oluşturuldu!\"\n");
        prompt.append("\n");
