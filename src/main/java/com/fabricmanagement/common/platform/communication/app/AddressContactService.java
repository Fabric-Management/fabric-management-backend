 ...
 *
 * <p>Handles Many-to-Many relationship between Address and Contact.
 *
 * <p>Supports address-specific contacts for both Company and User addresses.
 *
 * <p>Key responsibilities:
 *
 * <ul>
 *   <li>Assign contacts to addresses
 *   <li>Remove address-contact assignments
 *   <li>Manage primary contact per address
 *   <li>Query contacts for addresses
 * </ul>
 */
@Service
...
public class AddressContactService {
  ...
  public List<AddressContact> getContactsForAddresses(List<UUID> addressIds) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.debug(
        "Batch loading contacts for addresses: tenantId={}, count={}", tenantId, addressIds.size());
    return addressContactRepository.findByTenantIdAndAddressIdIn(tenantId, addressIds);
  }

  @Transactional
  public AddressContact assignContact(
      UUID addressId, UUID contactId, Boolean isPrimary, String label) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.info(
        "Assigning contact to address: tenantId={}, addressId={}, contactId={}, isPrimary={}, label={}",
        tenantId,
        addressId,
        contactId,
        isPrimary,
    ...
    if (Boolean.TRUE.equals(isPrimary)) {
      addressContactRepository
          .findPrimaryByAddressId(addressId)
          .ifPresent(
              existing -> {
  ...
  public void removeContact(UUID addressId, UUID contactId) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.info(
        "Removing contact from address: tenantId={}, addressId={}, contactId={}",
        tenantId,
        addressId,
        contactId);
    ...
    AddressContact addressContact =
        addressContactRepository
            .findByAddressIdAndContactId(addressId, contactId)
  ...
  public AddressContact setAsPrimary(UUID addressId, UUID contactId) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.info(
        "Setting primary contact: tenantId={}, addressId={}, contactId={}",
        tenantId,
        addressId,
        contactId);
  ...
  public AddressContact updateLabel(UUID addressId, UUID contactId, String label) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.debug(
        "Updating contact label: tenantId={}, addressId={}, contactId={}, label={}",
        tenantId,
        addressId,
        contactId,
        label);
