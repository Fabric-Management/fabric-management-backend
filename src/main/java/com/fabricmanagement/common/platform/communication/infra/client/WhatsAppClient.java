 ...
 *
 * <p>Handles communication with Meta WhatsApp Business API for verification codes.</p>
 *
 * <p><b>Features:</b></p>
 * <ul>
 *   <li>✅ Phone number capability check</li>
 *   <li>✅ Verification code delivery via template messages</li>
...
public class WhatsAppClient {
     ...
     * <p><b>Fail-safe:</b> Returns false if check fails (assumes no WhatsApp).</p>
     *
    ...
    public boolean phoneHasWhatsApp(String phoneNumber) {
        if (!whatsAppEnabled || phoneNumber == null || phoneNumber.isBlank()) {
            log.debug("WhatsApp check skipped: enabled={}, phone={}", 
                whatsAppEnabled, maskPhone(phoneNumber));
            return false;
        }
        ...
        try {
            ...
            ResponseEntity<WhatsAppPhoneNumberResponse> response = getRestTemplate().exchange(
            ...
            if (body != null && body.getCapabilities() != null) {
                boolean hasWhatsApp = Boolean.TRUE.equals(
                    body.getCapabilities().getCanReceiveWhatsAppMessages()
                );
                log.debug("WhatsApp capability check result: phone={}, hasWhatsApp={}", 
                    maskPhone(phoneNumber), hasWhatsApp);
                return hasWhatsApp;
            }
            ...
            log.warn("Failed to check WhatsApp capability for: {} - {}", 
     ...
     *
     * <p>This method attempts to verify the recipient's phone number directly.</p>
     * <p><b>Note:</b> This may require additional API permissions.</p>
     */
     ...
     *
     * <p>Uses WhatsApp template messages for verification code delivery.</p>
     * <p><b>Template Requirements:</b></p>
     * <ul>
     *   <li>Template name: configured via {@code verification-template-name}</li>
     *   <li>Template language: "en" (English)</li>
     *   <li>Template must have a body parameter for the code</li>
     * </ul>
     *
     * @param phoneNumber E.164 format phone number (e.g., +14155551234)
    ...
    public void sendVerificationCode(String phoneNumber, String code) {
        ...
        try {
            ...
            ResponseEntity<WhatsAppMessageResponse> response = getRestTemplate().postForEntity(
            ...
            if (responseBody != null && responseBody.getMessages() != null && 
                !responseBody.getMessages().isEmpty()) {
                messageId = responseBody.getMessages().get(0).getId();
            }

            log.info("✅ WhatsApp verification code sent successfully: messageId={}, recipient={}", 
                messageId, maskPhone(phoneNumber));

        } catch (RestClientException e) {
            log.error("❌ Failed to send WhatsApp verification code to: {} - {}", 
                maskPhone(phoneNumber), e.getMessage(), e);
            throw new RuntimeException("WhatsApp sending failed: " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("❌ Unexpected error sending WhatsApp verification code: {}", 
                e.getMessage(), e);
            throw new RuntimeException("WhatsApp sending failed: " + e.getMessage(), e);
        }
    }
     ...
     *
     * <p>Checks if WhatsApp Business API is accessible and configured.</p>
     *
