...
/**
 * Flat Number Extractor - Extracts flat/apartment numbers from formatted addresses
 * when Google Maps API doesn't provide subpremise component.
 * 
 * <p>Uses region-specific patterns to extract flat numbers from formatted address strings.
 * This is a fallback mechanism for countries where Google doesn't reliably return subpremise.</p>
 */
@Slf4j
@UtilityClass
public class FlatNumberExtractor {
    ...
    static {
        // GLOBAL PATTERN (all countries) - PRIORITY: Check for combined format first (20/34, 20 34, 20-34)
        // This pattern is used in many countries: Turkey, Germany, France, Italy, Spain, etc.
        PATTERNS.add(new FlatNumberPattern(
            null, // null = applies to all countries
            Pattern.compile("(?i)\\b(\\d+[A-Za-z]?)\\s*[/\\s-]+\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("TR"),
            Pattern.compile("(?i)\\b(?:Kat|K|K\\.)\\s*(\\d+)\\s*(?:Daire|D|Da)\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
            "Turkish (Kat 3 Daire 5)"
        ));
        PATTERNS.add(new FlatNumberPattern(
            List.of("TR"),
            Pattern.compile("(?i)\\b(?:Daire|D|Da|D\\.)\\s*:?\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("GB", "IE"),
            Pattern.compile("(?i)\\b(?:Flat|Fl\\.?|Apt|Apartment|Unit|U\\.?)\\s*(?:No\\.?\\s*)?([A-Z]?\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("US", "CA"),
            Pattern.compile("(?i)\\b(?:Apt|Apartment|Unit|U\\.?|Suite|Ste\\.?|#)\\s*(?:No\\.?\\s*)?([A-Z]?\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("AU", "NZ"),
            Pattern.compile("(?i)\\b(?:Unit|U\\.?|Flat|Apt|Apartment)\\s*(?:No\\.?\\s*)?(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("SG", "HK"),
            Pattern.compile("(?i)(?:#|Flat|Unit)\\s*(\\d+-\\d+|[A-Z]\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("DE", "AT", "CH"),
            Pattern.compile("(?i)\\b(?:Wohnung|Whg\\.?|Apt|App\\.?)\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("FR", "BE", "LU"),
            Pattern.compile("(?i)\\b(?:Appartement|Apt\\.?|App\\.?|Étage|Etg\\.?)\\s*(?:N°|No\\.?)?\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            List.of("ES", "IT", "PT"),
            Pattern.compile("(?i)\\b(?:Piso|Planta|Apt\\.?|App\\.?|Piano)\\s*(?:N°|No\\.?)?\\s*(\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
        ...
        PATTERNS.add(new FlatNumberPattern(
            ...
            Pattern.compile("(?i)\\b(?:Flat|Apt|Apartment|Unit|Suite|#)\\s*(?:No\\.?\\s*)?([A-Z]?\\d+[A-Za-z]?)\\b", Pattern.CASE_INSENSITIVE),
    ...
    public static String extractFlatNumber(String formattedAddress, String countryCode) {
        ...
        for (FlatNumberPattern pattern : PATTERNS) {
            boolean matchesCountry = pattern.countryCodes == null 
                || (countryCode != null && !countryCode.isBlank() && pattern.countryCodes.contains(countryCode));
            ...
            if (matchesCountry) {
                ...
                if (matcher.find()) {
                    ...
                    if (matcher.groupCount() >= 2 && matcher.group(2) != null && !matcher.group(2).isBlank()) {
                        // Pattern with 2 groups: combined format (20/34) or Kat/Daire pattern
                        String patternDesc = pattern.description.toLowerCase();
                        if (patternDesc.contains("combined format") || patternDesc.contains("street+flat") 
                            || patternDesc.contains("20/34") || patternDesc.contains("global")) {
                            ...
                            if (isValidFlatNumber(firstNum, secondNum)) {
                                ...
                                log.debug("⚠️ Skipped invalid flat number pattern: '{}'/'{}' (country: {})", 
                                    firstNum, secondNum, countryCode);
                    ...
                    if (extracted != null && !extracted.isBlank()) {
                        log.debug("✅ Extracted flat number '{}' from formatted address using pattern: {} (country: {})", 
                            extracted, pattern.description, countryCode);
    ...
    private static boolean isValidFlatNumber(String firstNum, String secondNum) {
        try {
            ...
            // - Between 1 and 9999 (reasonable apartment/flat numbers - some buildings have 4-digit flat numbers)
