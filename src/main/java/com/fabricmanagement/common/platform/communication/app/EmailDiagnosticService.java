 ...
 *
 * <p>Provides comprehensive diagnostics for email delivery issues.
 */
@Service
...
public class EmailDiagnosticService {
  ...
  private Map<String, Object> checkEmailQueue() {
    ...
    long sendingCount =
        emailOutboxRepository.countByStatusAndIsActiveTrue(EmailOutboxStatus.SENDING);
  ...
  private List<String> generateRecommendations(Map<String, Object> report) {
    ...
    if (!Boolean.TRUE.equals(config.get("useOutbox"))) {
      recommendations.add(
          "⚠️ Enable email outbox for reliable email delivery: application.email.use-outbox=true");
    }
    if (!Boolean.TRUE.equals(config.get("emailOutboxWorkerEnabled"))) {
      recommendations.add(
          "⚠️ Enable email outbox worker: application.email.outbox.worker-enabled=true");
    }
    ...
    if (pendingCount > 10) {
      recommendations.add(
          "⚠️ High pending email count: " + pendingCount + ". Check if background job is running.");
    }
    if (stuckCount > 0) {
      recommendations.add(
          "🚨 CRITICAL: "
              + stuckCount
              + " email(s) stuck in SENDING status. Manual intervention required.");
    }
    if (failedCount > 5) {
      recommendations.add(
          "🚨 CRITICAL: "
              + failedCount
              + " failed email(s). Check SMTP configuration and network connectivity.");
    }
