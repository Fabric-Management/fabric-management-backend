...
/**
 * Address Component Mapper - Maps Google Maps API address components to domain fields
 * based on region-specific address structures.
 * 
 * <p>Each region has different address component hierarchies. This mapper applies
 * the correct mapping strategy based on the country's region group.</p>
 */
@Slf4j
public class AddressComponentMapper {
    ...
    public AddressComponents map(PlaceDetailsResponse placeDetails, String originalInput) {
        ...
        List<PlaceDetailsResponse.PlaceAddressComponent> components = placeDetails.getAddressComponents();
        log.debug("🔍 AddressComponentMapper: Processing {} address components (Places API)", components.size());

        AddressComponents result = new AddressComponents();

        Map<String, PlaceDetailsResponse.PlaceAddressComponent> componentMap = buildComponentMap(components);

        String countryCode = extractCountryCodeFromPlaceDetails(componentMap);
        AddressRegionMapper.AddressRegion region = AddressRegionMapper.getRegionByCountryCode(countryCode);
        ...
        if ((result.getFlatNumber() == null || result.getFlatNumber().isBlank()) 
            ...
            String extracted = extractFlatNumberFromOriginalInput(originalInput, result.getStreetAddress(), countryCode);
            if (extracted != null && !extracted.isBlank()) {
                result.setFlatNumber(extracted);
                log.debug("  ✅ ORIGINAL INPUT: Extracted flatNumber='{}' from originalInput='{}'", extracted, originalInput);
            }
        }
    ...
    public AddressComponents map(GeocodingResponse.GeocodingResult geocodingResult) {
        ...
        log.debug("🔍 AddressComponentMapper: Processing {} address components (Geocoding API)", components.size());
    ...
    private Map<String, PlaceDetailsResponse.PlaceAddressComponent> buildComponentMap(
        ...
        return components.stream()
            ...
            .flatMap(comp -> comp.getTypes().stream()
                .map(type -> Map.entry(type, comp)))
    ...
    private String extractCountryCodeFromPlaceDetails(Map<String, PlaceDetailsResponse.PlaceAddressComponent> componentMap) {
    ...
    private String extractCountryCodeFromGeocoding(List<GeocodingResponse.AddressComponent> components) {
            ...
            if (types.contains("street_number")) {
                ...
                if (result.getFlatNumber() == null || result.getFlatNumber().isBlank()) {
                    ...
                    if (routeParts != null && routeParts.length == 3) {
                        ...
                        log.debug("  ✅ PARSED: Route '{}' contains flat number → streetNumber='{}', flatNumber='{}', route='{}'", 
                            longText, streetNum, flatNum, routeName);
                        continue;
                    }
                }
                ...
                if (!isUK) {
                    ...
                    log.debug("  ⏭️  SKIPPED: administrative_area_level_1 '{}' (UK/Ireland - not a state)", longText);
                }
    ...
    private void applyEasternEuropeMapping(
        ...
        for (PlaceDetailsResponse.PlaceAddressComponent comp : components) {
            ...
            if (types.contains("street_number")) {
                ...
                // Turkey/Eastern Europe: administrative_area_level_4/sublocality is mahalle/neighborhood (district)
                if (result.getDistrict() == null) {
                    result.setDistrict(longText);
        ...
        for (GeocodingResponse.AddressComponent comp : components) {
            ...
            if (types.contains("street_number")) {
                ...
                if (!isUK) {
                    ...
                    log.debug("  ⏭️  SKIPPED: administrative_area_level_1 '{}' (UK/Ireland - not a state)", longName);
                }
            } else if (types.contains("postal_code")) {
    ...
    /**
     * Set street address and detect flat number from combined format (e.g., "20/34", "20 34", "20-34").
     * 
    ...
    private void setStreetAddress(AddressComponents result, String value, boolean isFirst) {
        ...
        if (isFirst && (result.getFlatNumber() == null || result.getFlatNumber().isBlank())) {
            ...
            if (parts != null && parts.length == 2) {
                ...
                result.setFlatNumber(flatNum);
                log.debug("  ✅ PARSED: Street number '{}' contains flat number → streetNumber='{}', flatNumber='{}'", 
                    originalValue, streetNum, flatNum);
            }
        }
     ...
     */
    private void applyFlatNumberFallback(AddressComponents result, String formattedAddress, String countryCode) {
        ...
        if (result.getFlatNumber() == null || result.getFlatNumber().isBlank()) {
            ...
            String combinedExtracted = extractCombinedFormatFromFormattedAddress(formattedAddress, result.getStreetAddress());
            if (combinedExtracted != null && !combinedExtracted.isBlank()) {
                result.setFlatNumber(combinedExtracted);
                log.debug("  ✅ FALLBACK: Extracted flatNumber='{}' from formattedAddress combined format", combinedExtracted);
                return;
            }
            ...
            if (extracted != null && !extracted.isBlank()) {
                ...
                if (streetAddress != null && !streetAddress.isBlank()) {
                    java.util.regex.Pattern streetNumPattern = java.util.regex.Pattern.compile("^\\s*(\\d+[A-Za-z]?)\\s+");
                    ...
                    if (streetMatcher.find()) {
                        ...
                        if (streetNum.equals(extractedClean)) {
                            log.debug("  ⚠️  REJECTED: Extracted flatNumber '{}' equals street number '{}' - likely false positive", 
                                extracted, streetNum);
                            extracted = null;
                        }
                    }
                ...
                if (extracted != null && !extracted.isBlank()) {
                    result.setFlatNumber(extracted);
                    log.debug("  ✅ FALLBACK: Extracted flatNumber='{}' from formatted address using pattern", extracted);
                }
            }
        } else {
     ...
     */
    private String extractCombinedFormatFromFormattedAddress(String formattedAddress, String streetAddress) {
        if (formattedAddress == null || formattedAddress.isBlank() || streetAddress == null || streetAddress.isBlank()) {
            return null;
        }
        ...
        // Search for "knownStreetNum/XX" or "knownStreetNum XX" or "knownStreetNum-XX" pattern in formattedAddress
        ...
        String patternStr = "\\b(" + escapedStreetNum + "|" + escapedStreetNumClean + ")\\s*[/\\s-]+\\s*(\\d+[A-Za-z]?)\\b";
            ...
            if (!flatNumClean.equals(matchedStreetNumClean)) {
                try {
                    ...
                    if (flat >= 1 && flat <= 9999 && flat != street && street <= 100000) {
                        log.debug("  ✅ Found combined format in formattedAddress: '{}' -> flatNumber='{}' (street: {})", 
                            matcher.group(0), flatNum, matchedStreetNum);
                        return flatNum;
                    }
                } catch (NumberFormatException e) {
     ...
     */
    private String extractFlatNumberFromOriginalInput(String originalInput, String streetAddress, String countryCode) {
        ...
        java.util.regex.Pattern pattern = java.util.regex.Pattern.compile(globalPattern, java.util.regex.Pattern.CASE_INSENSITIVE);
        java.util.regex.Matcher matcher = pattern.matcher(originalInput);
