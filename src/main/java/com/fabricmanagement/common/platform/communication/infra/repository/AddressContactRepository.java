...
public interface AddressContactRepository extends JpaRepository<AddressContact, AddressContactId> {
  ...
  @Query(
      "SELECT ac FROM AddressContact ac WHERE ac.tenantId = :tenantId AND ac.addressId = :addressId")
  List<AddressContact> findByTenantIdAndAddressId(
      @Param("tenantId") UUID tenantId, @Param("addressId") UUID addressId);

  /** Find specific address-contact assignment. */
  @Query(
      "SELECT ac FROM AddressContact ac WHERE ac.addressId = :addressId AND ac.contactId = :contactId")
  Optional<AddressContact> findByAddressIdAndContactId(
      @Param("addressId") UUID addressId, @Param("contactId") UUID contactId);
  ...
  @Query(
      "SELECT ac FROM AddressContact ac WHERE ac.tenantId = :tenantId AND ac.addressId IN :addressIds")
  List<AddressContact> findByTenantIdAndAddressIdIn(
      @Param("tenantId") UUID tenantId, @Param("addressIds") List<UUID> addressIds);
}
