 ...
 *
 * <p><b>Security:</b> All endpoints require PLATFORM_ADMIN role.</p>
 *
 * <p><b>Purpose:</b> Allows platform administrators to:</p>
 * <ul>
 *   <li>View all tenants in the system</li>
 *   <li>Access any tenant's data (companies, users, etc.)</li>
 *   <li>Manage tenant operations</li>
 *   <li>Cross-tenant reporting</li>
 * </ul>
 *
 * <h2>Tenant Context Switching:</h2>
 * <p>Platform admin can switch tenant context using query parameter:</p>
 * <pre>
 * GET /api/admin/tenants/{tenantId}/users?tenantId={targetTenantId}
 * </pre>
 *
 * <p>Or use X-Tenant-Id header for automatic context switching (if implemented in interceptor).</p>
 */
@RestController
...
public class PlatformAdminController {
     ...
     *
     * <p><b>Security:</b> PLATFORM_ADMIN role required</p>
     *
    ...
    public ResponseEntity<ApiResponse<List<CompanyDto>>> getAllTenants() {
        ...
        return ResponseEntity.ok(ApiResponse.success(tenants, 
            String.format("Found %d tenants", tenants.size())));
    }
     ...
     *
     * <p><b>Security:</b> PLATFORM_ADMIN role required</p>
     * <p><b>Tenant Context:</b> Automatically switches to target tenant context</p>
     *
    ...
    public ResponseEntity<ApiResponse<List<UserDto>>> getTenantUsers(@PathVariable UUID tenantId) {
        ...
        return ResponseEntity.ok(ApiResponse.success(users, 
            String.format("Found %d users in tenant", users.size())));
    }
    ...
    @PreAuthorize("hasRole('PLATFORM_ADMIN')")
    public ResponseEntity<ApiResponse<List<CompanyDto>>> getTenantCompanies(@PathVariable UUID tenantId) {
        ...
        return ResponseEntity.ok(ApiResponse.success(companies, 
            String.format("Found %d companies in tenant", companies.size())));
    }
    ...
    @PreAuthorize("hasRole('PLATFORM_ADMIN')")
    public ResponseEntity<ApiResponse<TenantStatistics>> getTenantStatistics(@PathVariable UUID tenantId) {
     ...
     *
     * <p><b>Note:</b> This is informational - actual tenant switching happens
     * automatically via TenantContext.executeInTenantContext() in service methods.</p>
     *
    ...
    public ResponseEntity<ApiResponse<String>> switchTenantContext(@PathVariable UUID tenantId) {
        ...
        // Note: Actual tenant switching is handled per-request via TenantContext.executeInTenantContext()
