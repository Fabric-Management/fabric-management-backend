package com.fabricmanagement.common.platform.subscription.infra.repository;
 ...
 *
 * <p>Manages quota tracking for subscriptions (users, API calls, storage, etc.)
 */
@Repository
public interface SubscriptionQuotaRepository extends JpaRepository<SubscriptionQuota, UUID> {
   ...
   *
   * <p>Used by scheduled jobs to reset monthly/daily quotas.
   *
   ...
   */
  @Query(
      "SELECT q FROM SubscriptionQuota q WHERE q.tenantId = :tenantId AND q.quotaUsed >= q.quotaLimit")
   ...
   */
  @Query(
      "SELECT CASE WHEN q.quotaLimit > q.quotaUsed "
          + "THEN (q.quotaLimit - q.quotaUsed) ELSE 0 END "
          + "FROM SubscriptionQuota q "
          + "WHERE q.tenantId = :tenantId AND q.quotaType = :quotaType")
  Optional<Long> getRemainingQuota(
      @Param("tenantId") UUID tenantId, @Param("quotaType") String quotaType);
}
