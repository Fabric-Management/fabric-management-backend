 ...
 *
 * <p>Manages the master catalog of all available features in the system.
 */
@Repository
public interface FeatureCatalogRepository extends JpaRepository<FeatureCatalog, UUID> {
  ...
  @Query(
      value =
          "SELECT * FROM common_company.common_feature_catalog "
  ...
  @Query(
      value =
          "SELECT * FROM common_company.common_feature_catalog "
              ...
              + "OR available_in_tiers::jsonb @> CAST(:tier AS jsonb))",
      nativeQuery = true)
  List<FeatureCatalog> findByOsCodeAndTier(
      @Param("osCode") String osCode, @Param("tier") String tier);
