 ...
 * Responsibilities:
 * <ul>
 *   <li>Generate single-use verification codes with BCrypt hashing</li>
 *   <li>Enforce per-contact, per-tenant, and global throttling rules</li>
 *   <li>Validate codes with attempt tracking and audit logging</li>
 *   <li>Provide consistent error messaging and logging for security monitoring</li>
 * </ul>
 */
@Service
...
public class VerificationCodeManager {
    ...
    public IssuedVerificationCode issueCode(String contactValue, VerificationType type) {
        ...
        log.info("Verification code issued: tenantId={}, contact={}, type={}, expiresAt={}",
            tenantId,
    ...
    public void validateAndConsume(String contactValue, VerificationType type, String rawCode) {
        ...
        VerificationCode verificationCode = verificationCodeRepository
            .findTopByTenantIdAndContactValueAndTypeOrderByCreatedAtDesc(tenantId, contactValue, type)
            .orElseThrow(() -> {
                log.warn("Verification code not found: tenantId={}, contact={}, type={}",
                    tenantId, maskContact(contactValue), type);
                return new IllegalArgumentException("Verification code is invalid or expired");
            });

        if (verificationCode.isExpired()) {
            log.warn("Verification code expired: tenantId={}, contact={}, type={}",
                tenantId, maskContact(contactValue), type);
        ...
        if (Boolean.TRUE.equals(verificationCode.getIsUsed())) {
            log.warn("Verification code already used: tenantId={}, contact={}, type={}",
                tenantId, maskContact(contactValue), type);
        ...
        if (verificationCode.getAttemptCount() >= maxAttempts) {
            log.warn("Verification code attempt limit reached: tenantId={}, contact={}, type={}",
                tenantId, maskContact(contactValue), type);
        ...
        if (!verificationCode.matches(rawCode, passwordEncoder)) {
            ...
            verificationCodeRepository.save(verificationCode);
            log.warn("Verification code mismatch: tenantId={}, contact={}, type={}, attempts={}", tenantId,
                maskContact(contactValue), type, verificationCode.getAttemptCount());
            throw new IllegalArgumentException("Verification code is invalid or expired");
        }

        verificationCode.markAsUsed();
        verificationCodeRepository.save(verificationCode);
        log.info("Verification code validated: tenantId={}, contact={}, type={}",
            tenantId, maskContact(contactValue), type);
    }

    private void enforceRateLimits(UUID tenantId, String contactValue, VerificationType type, Instant windowStart) {
        long perContact = verificationCodeRepository
            .countByTenantIdAndContactValueAndTypeAndCreatedAtAfter(tenantId, contactValue, type, windowStart);
        if (perContact >= maxPerContactWindow) {
            log.warn("Verification throttle exceeded (contact): tenantId={}, contact={}, type={}",
                tenantId, maskContact(contactValue), type);
        ...
        long perTenant = verificationCodeRepository
            .countByTenantIdAndTypeAndCreatedAtAfter(tenantId, type, genericWindowStart);
        if (perTenant >= maxPerTenantWindow) {
            log.warn("Verification throttle exceeded (tenant): tenantId={}, type={}", tenantId, type);
            throw new IllegalArgumentException("Verification temporarily limited for your organisation. Please try later.");
        }
