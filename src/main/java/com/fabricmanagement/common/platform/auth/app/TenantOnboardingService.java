 ...
 *
 * <p><b>CRITICAL:</b> This service handles the SPECIAL case where tenant_id = company_id
 *
 * <h2>Used in two flows:</h2>
 *
 * <ul>
 *   <li><b>Sales-led:</b> Internal sales team creates tenant (admin endpoint)
 *   <li><b>Self-service:</b> Public user signs up (public endpoint)
 * </ul>
 *
 * <h2>What it creates:</h2>
 *
 * <ol>
 *   <li>Company (with tenant_id = company_id)
 *   <li>Admin User (pre-approved)
 *   <li>Selected OS Subscriptions (trial)
 *   <li>Registration Token (for password setup)
 * </ol>
 */
@Service
...
public class TenantOnboardingService {
   ...
   *
   * <p><b>CRITICAL:</b> Sets tenant_id = company_id for root tenant
   *
  ...
  public TenantOnboardingResponse createSalesLedTenant(TenantOnboardingRequest request) {
    ...
    Company company =
        createTenantCompany(request.getCompanyName(), request.getTaxId(), request.getCompanyType());
    ...
    List<Subscription> subscriptions =
        createInitialSubscriptions(
            company.getId(), company.getTenantId(), selectedOS, request.getTrialDays());
    ...
    log.info(
        "✅ Sales-led tenant created - company: {}, admin: {}, token: {}",
        company.getUid(),
  ...
  public TenantOnboardingResponse createSelfServiceTenant(SelfSignupRequest request) {
    ...
    log.info(
        "✅ Self-service tenant created - company: {}, admin: {}, token: {}",
        company.getUid(),
        PiiMaskingUtil.maskEmail(adminContact),
        token.getToken());
   ...
   *
   * <p>Creates Address and Contact entities, then assigns them to company via junction tables.
   */
  private void addCompanyAddressAndContact(
      UUID companyId, UUID tenantId, TenantOnboardingRequest request) {
    addCompanyAddressAndContact(
        companyId,
        tenantId,
        request.getAddress(),
        request.getCity(),
  ...
  private void addCompanyAddressAndContact(
    ...
    log.debug(
        "Adding company address/contact: companyId={}, hasAddress={}, hasContact={}",
        companyId,
        hasAddressInfo,
        hasContactInfo);
    ...
    try {
      ...
      if (email != null) {
        ...
        companyContactService.assignContact(
            companyId, emailContact.getId(), isDefault, null // department
            );
   ...
   *
   * <p><b>CRITICAL:</b> This is the ONLY place where tenant_id equals company_id
   */
  private Company createTenantCompany(String name, String taxId, CompanyType type) {
    ...
    log.debug(
        "Tenant UID confirmed: {} for tenantId={}",
        finalCompany.getUid(),
   ...
   *
   * <p>User is created within tenant context.
   */
  private User createAdminUser(
      ...
      String contactValue,
      String department) {
    log.debug("Creating admin user: contact={}", PiiMaskingUtil.maskEmail(contactValue));
    ...
    return TenantContext.executeInTenantContext(
        ...
        () -> {
          ...
          userContactService.assignContact(
              saved.getId(), contact.getId(), true // isDefault
              );
          ...
          if (department != null) {
            ...
            departmentRepository
                .findByTenantIdAndCompanyIdAndDepartmentName(currentTenantId, companyId, department)
                .ifPresent(
                    dept -> {
          ...
          // Regular users created by admins will get roles from request
          ...
          log.info(
              "✅ Tenant admin role assigned from platform: userId={}, roleCode={}, roleId={}",
              saved.getId(),
              assignedRole.getRoleCode(),
              assignedRole.getId());
   ...
   *
   * <p>Simple model: Only creates selected OS subscriptions.
   *
   * <p>No mandatory OS - tenant chooses what they need.
   */
  private List<Subscription> createInitialSubscriptions(
      UUID companyId, UUID tenantId, List<String> selectedOS, int trialDays) {
  ...
  private RegistrationToken createRegistrationToken(
      String contactValue, RegistrationTokenType tokenType, UUID userId, UUID companyId) {
   ...
   *
   * <p>Priority: FRONTEND_URL → APP_BASE_URL → localhost fallback (dev only)
   */
   ...
   *
   * <p>Uses sync method to ensure email is queued before transaction commit. This prevents async
   * timing issues where email might not be queued.
   */
   ...
   *
   * <p><b>⚠️ IMPORTANT: This method is ONLY for tenant onboarding admin users.</b>
   *
   * <p><b>Regular users created by admins will get roles from request via
   * UserService.createInternalUser().</b>
   *
   * <p><b>Role Assignment Strategy:</b>
   *
   * <ol>
   *   <li><b>ADMIN role</b> (required for tenant admin) - Primary choice
   *   <li><b>Fallback roles</b> (only if ADMIN missing) - PROD_MANAGER → HR_MANAGER → Any active
   *       role
   *   <li><b>Exception</b> if no roles exist (system configuration error)
   * </ol>
   *
   * <p><b>Why fallback?</b>
   *
   * <ul>
   *   <li>✅ Ensures tenant onboarding succeeds even if ADMIN role temporarily missing
   *   <li>✅ User always has a role (security requirement)
   *   <li>✅ Mail sending is guaranteed (user created successfully)
   *   <li>⚠️ Fallback roles are logged as warnings for admin review
   * </ul>
   *
   * <p><b>Security Note:</b>
   *
   * <ul>
   *   <li>✅ Only called during tenant onboarding (createAdminUser)
   *   <li>✅ Regular users get roles from request (UserService handles this)
   *   <li>✅ Admin users cannot assign ADMIN role to others via this method
   * </ul>
   *
  ...
  private Role assignTenantAdminRole(User user, UUID systemTenantId, UUID tenantId) {
    ...
    if (adminRole.isPresent()) {
      log.debug(
          "✅ ADMIN role assigned to tenant admin: userId={}, tenantId={}", user.getId(), tenantId);
      return adminRole.get();
    }

    log.warn(
        "⚠️ ADMIN role not found in platform. Using fallback role for tenant admin... tenantId={}",
        tenantId);

    // Priority 2: PROD_MANAGER (has management capabilities - acceptable fallback)
    Optional<Role> prodManagerRole =
        roleRepository.findByTenantIdAndRoleCode(systemTenantId, "PROD_MANAGER");
    if (prodManagerRole.isPresent()) {
      log.warn(
          "⚠️ ADMIN role not found, assigned PROD_MANAGER as fallback for tenant admin: "
    ...
    Optional<Role> hrManagerRole =
        roleRepository.findByTenantIdAndRoleCode(systemTenantId, "HR_MANAGER");
    if (hrManagerRole.isPresent()) {
      log.warn(
          "⚠️ ADMIN and PROD_MANAGER roles not found, assigned HR_MANAGER as fallback for tenant admin: "
              + "userId={}, tenantId={}. Please ensure ADMIN role exists in platform.",
          user.getId(),
          tenantId);
    ...
    if (!allRoles.isEmpty()) {
      ...
      log.error(
          "❌ ADMIN role not found, assigned '{}' as fallback for tenant admin: "
              + "userId={}, tenantId={}. This is a configuration issue - ADMIN role should exist in platform.",
          fallbackRole.getRoleCode(),
          user.getId(),
          tenantId);
      return fallbackRole;
    }
    ...
    log.error(
        "❌ CRITICAL: No platform roles found in system tenant. "
            + "Migration V013 must be executed before tenant creation. tenantId={}",
        tenantId);
   ...
   *
   * <p>Format: {COMPANY_NAME_PREFIX}-{SEQUENCE} Example: "Akkayalar Tekstil Dokuma San.Tic.Ltd.Sti"
   * → "AKKAYALAR-001"
   *
   * <p>Algorithm:
   *
   * <ol>
   *   <li>Extract first word from company name (uppercase, max 10 chars)
   *   <li>Remove special characters
   *   <li>Add sequence number (001, 002, etc.) - check uniqueness if needed
   * </ol>
   *
  ...
  private String generateTenantUidFromCompanyName(String companyName) {
    ...
    do {
      ...
      if (counter > 999) {
        ...
        log.warn(
            "Too many collisions for prefix {} (>{}, using UUID suffix: {}",
            firstWord,
            999,
            candidateUid);
        break;
