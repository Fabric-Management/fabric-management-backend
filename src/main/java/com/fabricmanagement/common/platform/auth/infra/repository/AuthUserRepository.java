 ...
 *
 * <p><b>CRITICAL DESIGN CHANGE:</b> User-based authentication (one AuthUser per User).</p>
 * <p>Multi-contact login supported: Any verified contact of a User can be used for login.</p>
 */
@Repository
public interface AuthUserRepository extends JpaRepository<AuthUser, UUID> {
     ...
     * <p>Finds User via Contact → UserContact → User, then returns User's AuthUser.</p>
     ...
     * <p>Used for batch checking if a User has any AuthUser via their contacts.</p>
     * <p><b>CRITICAL:</b> Tenant isolation enforced via Contact's tenant_id.</p>
     * 
     * @param userId User ID
     ...
     * <p><b>CRITICAL:</b> Tenant isolation enforced via User's tenant_id.</p>
     * 
     * @param userIds List of User IDs to check
     * @return Set of User IDs that have AuthUser
     */
    @Query("SELECT au.userId FROM AuthUser au " +
           "JOIN com.fabricmanagement.common.platform.user.domain.User u ON au.userId = u.id " +
     ...
     */
    @Deprecated
    @Query("SELECT au.contactId FROM AuthUser au " +
           ...
           "AND au.tenantId = c.tenantId")
    java.util.Set<UUID> findContactIdsByContactIds(@Param("contactIds") java.util.List<UUID> contactIds);
    ...
    @Query("SELECT au FROM AuthUser au " +
           ...
           "WHERE au.contactId IN :contactIds " +
           "AND au.tenantId = c.tenantId")
    java.util.List<AuthUser> findAllByContactIdIn(@Param("contactIds") java.util.List<UUID> contactIds);
}
