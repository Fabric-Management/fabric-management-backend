...
public interface CompanyRepository extends JpaRepository<Company, UUID> {
  ...
  List<Company> findByTenantIdAndCompanyTypeIn(
      @Param("tenantId") UUID tenantId, @Param("types") List<CompanyType> types);

  /**
   * Find all root tenant companies (where id = tenantId). Used by platform admin to list all
   * tenants in the system.
   *
   * <p><b>Performance:</b> Filters at database level instead of loading all companies and filtering
   * in Java. This is critical for performance when there are many companies.
   *
   * <p><b>Note:</b> Only includes tenant-type company types (SPINNER, WEAVER, KNITTER, etc.)
   *
  ...
  /**
   * Find company by UID (global lookup, not tenant-scoped). Used for tenant UID uniqueness checking
