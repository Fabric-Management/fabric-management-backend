...
public interface PositionRepository extends JpaRepository<Position, UUID> {
     ...
     * 
     * <p><b>Optimized:</b> JOIN FETCH to avoid N+1 queries for defaultRole and hierarchicalParent.</p>
     */
     ...
     */
    @Query("SELECT p FROM Position p WHERE p.departmentId = :departmentId AND p.isActive = true ORDER BY p.displayOrder, p.positionName")
    ...
    Optional<Position> findByTenantIdAndPositionCode(@Param("tenantId") UUID tenantId, @Param("positionCode") String positionCode);

    /**
     * Check if position exists by tenant ID and position code.
     */
    @Query("SELECT COUNT(p) > 0 FROM Position p WHERE p.tenantId = :tenantId AND p.positionCode = :positionCode")
    boolean existsByTenantIdAndPositionCode(@Param("tenantId") UUID tenantId, @Param("positionCode") String positionCode);
}
