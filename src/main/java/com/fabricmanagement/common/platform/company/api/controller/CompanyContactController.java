...
import com.fabricmanagement.common.platform.company.dto.CompanyContactDto;
...
public class CompanyContactController {
  ...
  public ResponseEntity<ApiResponse<List<CompanyContactDto>>> getCompanyContacts(
    ...
    List<CompanyContactDto> contacts =
        companyContactAssignmentService.getCompanyContacts(companyId).stream()
  ...
  @GetMapping("/default")
  public ResponseEntity<ApiResponse<CompanyContactDto>> getDefaultContact(
      @PathVariable UUID companyId) {
    log.debug("Getting default contact: companyId={}", companyId);

    CompanyContact defaultContact =
        companyContactAssignmentService
  ...
  public ResponseEntity<ApiResponse<List<CompanyContactDto>>> getDepartmentContacts(
      @PathVariable UUID companyId, @PathVariable String department) {
    log.debug("Getting department contacts: companyId={}, department={}", companyId, department);

    List<CompanyContactDto> contacts =
        companyContactAssignmentService.getDepartmentContacts(companyId, department).stream()
            .map(CompanyContactDto::from)
            .collect(Collectors.toList());
  ...
  public ResponseEntity<ApiResponse<CompanyContactDto>> assignContact(
      @PathVariable UUID companyId, @Valid @RequestBody AssignContactRequest request) {
    log.info(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(
            CompanyContactDto.from(companyContact), "Contact assigned successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<CompanyContactDto>> createAndAssignContact(
    ...
    log.info(
        "Creating and assigning contact to company: companyId={}, type={}",
        companyId,
    ...
    CompanyContact companyContact =
        companyContactAssignmentService.assignContact(
            companyId, contact.getId(), isDefault, department);

    return ResponseEntity.ok(
        ApiResponse.success(
            CompanyContactDto.from(companyContact), "Contact created and assigned successfully"));
  }

  @PutMapping("/{contactId}/default")
  public ResponseEntity<ApiResponse<CompanyContactDto>> setAsDefault(
      @PathVariable UUID companyId, @PathVariable UUID contactId) {
    log.info("Setting default contact: companyId={}, contactId={}", companyId, contactId);

    CompanyContact companyContact =
        companyContactAssignmentService.setAsDefault(companyId, contactId);

    return ResponseEntity.ok(
        ApiResponse.success(
            CompanyContactDto.from(companyContact), "Default contact set successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> removeContact(
    ...
    companyContactAssignmentService.removeContact(companyId, contactId);
