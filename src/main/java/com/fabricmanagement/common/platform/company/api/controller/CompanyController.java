...
public class CompanyController {
  ...
  public ResponseEntity<ApiResponse<SubscriptionDto>> activateSubscription(
      @PathVariable UUID id, @PathVariable UUID subscriptionId) {
  ...
  public ResponseEntity<ApiResponse<SubscriptionQuotaDto>> resetQuota(
      @PathVariable UUID subscriptionId, @PathVariable String quotaType) {
   ...
   *
   * <p><b>Public endpoint</b> - No authentication required (useful for signup forms)
   *
   * <p>Returns all company types with metadata (category, tenant status, suggested OS).
   */
  @GetMapping("/types")
  public ResponseEntity<ApiResponse<List<CompanyTypeDto>>> getCompanyTypes() {
    ...
    List<CompanyTypeDto> types =
        List.of(CompanyType.values()).stream().map(CompanyTypeDto::from).toList();
   ...
   *
   * <p>Returns only company types that can be platform tenants. Perfect for signup forms where
   * users select their company type.
   *
  ...
  public ResponseEntity<ApiResponse<List<CompanyTypeDto>>> getTenantCompanyTypes() {
    ...
    List<CompanyTypeDto> tenantTypes =
        List.of(CompanyType.values()).stream()
   ...
   * <p><b>Public endpoint</b> - No authentication required
   *
   * <p>Filter company types by category (TENANT, SUPPLIER, SERVICE_PROVIDER, PARTNER, CUSTOMER).
   *
