 ...
 *
 * <p>Handles categorization of departments (Production, Administrative, Utility, etc.)
 *
 * <p>Key responsibilities:
 *
 * <ul>
 *   <li>Category CRUD with tenant isolation
 *   <li>Category name uniqueness validation
 *   <li>Display order management
 * </ul>
 */
@Service
...
public class DepartmentCategoryService {
  ...
  public List<DepartmentCategory> findAll() {
    ...
    List<DepartmentCategory> systemCategories =
        categoryRepository.findByTenantIdAndIsActiveTrueOrderByDisplayOrderAsc(systemTenantId);
    List<DepartmentCategory> tenantCategories =
        categoryRepository.findByTenantIdAndIsActiveTrueOrderByDisplayOrderAsc(tenantId);
  ...
  public Optional<DepartmentCategory> findById(UUID categoryId) {
    ...
    UUID systemTenantId = TenantContext.SYSTEM_TENANT_ID;
    Optional<DepartmentCategory> systemCategory =
        categoryRepository.findByTenantIdAndId(systemTenantId, categoryId);
  ...
  public DepartmentCategory create(String categoryName, String description, Integer displayOrder) {
    ...
    DepartmentCategory category =
        DepartmentCategory.create(categoryName, description, displayOrder);
    DepartmentCategory saved = categoryRepository.save(category);

    log.info(
        "Department category created: id={}, uid={}, name={}",
        saved.getId(),
        saved.getUid(),
  ...
  @Transactional
  public DepartmentCategory update(
      UUID categoryId, String categoryName, String description, Integer displayOrder) {
    UUID tenantId = TenantContext.getCurrentTenantId();
    log.info("Updating department category: tenantId={}, categoryId={}", tenantId, categoryId);

    DepartmentCategory category =
        categoryRepository
            .findByTenantIdAndId(tenantId, categoryId)
  ...
  public void deactivate(UUID categoryId) {
    ...
    log.warn(
        "Department category deactivated: id={}, name={}",
        category.getId(),
