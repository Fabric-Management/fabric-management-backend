 ...
 *
 * <p>Implements CompanyFacade for cross-module communication.
 *
 * <p>Handles:
 *
 * <ul>
 *   <li>Company CRUD operations
 *   <li>Tenant management
 *   <li>Company hierarchy
...
public class CompanyService implements CompanyFacade {
  ...
  public CompanyDto createCompany(CreateCompanyRequest request) {
    ...
    if (saved.hasParent()) {
      ...
      if (checkParent != null && saved.getId().equals(checkParent.getParentCompanyId())) {
        ...
        log.error(
            "⚠️ Circular reference detected! Company {} and {} are each other's parent. "
  ...
  public List<CompanyDto> getCompaniesByType(CompanyType companyType) {
    ...
    return companyRepository.findByTenantIdAndCompanyType(tenantId, companyType).stream()
        .map(CompanyDto::from)
        .toList();
  }
   ...
   * <ul>
   *   <li>Company creation
   *   <li>Contact creation (email/phone) and assignment
   *   <li>Address creation and assignment
   * </ul>
   *
   * <p>All operations are atomic - if any step fails, entire transaction rolls back.
   *
   * <p>Benefits:
   *
   * <ul>
   *   <li>Single HTTP request instead of multiple
   *   <li>Better UX - one form submission
   *   <li>Transactional safety
   * </ul>
   */
  @Transactional
