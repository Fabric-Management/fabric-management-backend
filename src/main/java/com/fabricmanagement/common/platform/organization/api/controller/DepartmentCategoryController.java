...
import com.fabricmanagement.common.platform.organization.app.DepartmentCategoryService;
...
public class DepartmentCategoryController {
  ...
  public ResponseEntity<ApiResponse<List<DepartmentCategoryDto>>> getAllCategories() {
    ...
    List<DepartmentCategoryDto> categories =
  ...
  public ResponseEntity<ApiResponse<DepartmentCategoryDto>> createCategory(
    ...
    DepartmentCategory created =
        categoryService.create(
            request.getCategoryName(), request.getDescription(), request.getDisplayOrder());

    return ResponseEntity.ok(
        ApiResponse.success(
            DepartmentCategoryDto.from(created), "Department category created successfully"));
  }

  @PutMapping("/{id}")
  public ResponseEntity<ApiResponse<DepartmentCategoryDto>> updateCategory(
      @PathVariable UUID id, @Valid @RequestBody CreateDepartmentCategoryRequest request) {
    log.info("Updating department category: id={}", id);

    DepartmentCategory updated =
        categoryService.update(
            id, request.getCategoryName(), request.getDescription(), request.getDisplayOrder());

    return ResponseEntity.ok(
        ApiResponse.success(
            DepartmentCategoryDto.from(updated), "Department category updated successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> deactivateCategory(@PathVariable UUID id) {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(null, "Department category deactivated successfully"));
  }
