 ...
 *
 * <p>Used to detect and log duplicate names/keys in lists that are sent to frontend, preventing
 * React key conflicts and data integrity issues.
 *
 * <p><b>Usage:</b>
 *
 * <pre>{@code
...
public class DuplicateValidator {
  ...
  /**
   * Validates that all items in a list have unique keys based on a key extractor function. Logs
   * warnings for duplicates but does not throw exceptions (non-blocking).
   *
   * @param <T> Type of items in the list
   * @param items List of items to validate
   * @param keyExtractor Function to extract the key from each item (e.g.,
   *     PositionDto::getPositionName)
   * @param entityType Human-readable entity type name for logging (e.g., "positions",
   *     "departments")
   * @return true if no duplicates found, false if duplicates exist
   */
  public static <T> boolean validateAndLog(
      List<T> items, Function<T, String> keyExtractor, String entityType) {
    if (items == null || items.isEmpty()) {
      return true;
    }

    // Group items by key and find duplicates
    Map<String, List<T>> groupedByKey = items.stream().collect(Collectors.groupingBy(keyExtractor));

    // Find keys with more than one item
    Map<String, List<T>> duplicates =
    ...
    log.warn(
        "⚠️ Found {} duplicate key(s) in {} (total items: {}):",
        duplicates.size(),
        entityType,
        items.size());
   ...
   */
  public static <T> void validateOrThrow(
      List<T> items, Function<T, String> keyExtractor, String entityType) {
  ...
  /**
   * Extracts a human-readable string representation of an item for logging. Attempts to extract ID
   * and name fields using reflection-like approach.
   */
