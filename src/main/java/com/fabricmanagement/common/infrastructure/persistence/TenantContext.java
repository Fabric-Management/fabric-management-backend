 ...
 *
 * <p>Provides a thread-safe way to store and retrieve the current tenant ID throughout the request
 * lifecycle. This is used for:
 *
 * <ul>
 *   <li>Automatic tenant_id injection in {@link BaseEntity}
 *   <li>Row-Level Security (RLS) filtering in PostgreSQL
 *   <li>Multi-tenant data isolation
 *   <li>Audit logging with tenant context
 * </ul>
 *
 * <h2>Usage Flow:</h2>
 *
 * <ol>
 *   <li>Request arrives with JWT token
 *   <li>TenantFilter extracts tenant_id from JWT
 *   <li>TenantContext.setCurrentTenantId(tenantId) is called
 *   <li>All subsequent operations use this tenant context
 *   <li>TenantContext.clear() is called at request end
 * </ol>
 *
 * <h2>Example Usage:</h2>
 *
 ...
 * <h2>Thread Safety:</h2>
 *
 * <p>Uses {@link ThreadLocal} to ensure each thread has its own isolated tenant context. This is
 * safe in multi-threaded servlet containers.
 *
 * @see BaseEntity
 */
@Slf4j
public final class TenantContext {
  ...
  public static void setCurrentTenantCountry(String countryCode) {
    ...
    log.trace(
        "Set tenant country: {} for thread: {}", countryCode, Thread.currentThread().getName());
  }
   ...
   *
   * <p><b>IMPORTANT:</b> Always call this in a finally block to prevent context leakage!
   */
  public static void clear() {
    UUID tenantId = CURRENT_TENANT_ID.get();
    if (tenantId != null) {
      log.trace(
          "Clearing tenant context for thread: {}, tenant: {}",
          Thread.currentThread().getName(),
          tenantId);
    }
