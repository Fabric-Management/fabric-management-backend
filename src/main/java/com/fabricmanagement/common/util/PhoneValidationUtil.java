 ...
 *
 * <p><b>Purpose:</b> Validate phone numbers according to country-specific formats.
 *
 * <p><b>Features:</b>
 *
 * <ul>
 *   <li>✅ E.164 format validation (base check)
 *   <li>✅ Country-specific format validation
 *   <li>✅ Country code extraction
 *   <li>✅ Phone length validation per country
 * </ul>
 *
 * <p><b>Supported Countries:</b> TR, GB, US, DE, FR, IT, ES, NL, BE, CH, AT, SE, NO, DK, FI, PL,
 * CZ, GR, PT, IE
 */
@UtilityClass
@Slf4j
public class PhoneValidationUtil {
  ...
  static {
    ...
    COUNTRY_PATTERNS.put(
        "TR", new PhoneFormat("^\\+90[5][0-9]{9}$", 13, "Turkey: +90 5XX XXX XX XX"));
    COUNTRY_PATTERNS.put("GB", new PhoneFormat("^\\+44[0-9]{10,11}$", 13, "UK: +44 XXXX XXXXXX"));
    COUNTRY_PATTERNS.put(
        "US", new PhoneFormat("^\\+1[2-9][0-9]{2}[0-9]{7}$", 12, "US: +1 XXX XXX XXXX"));
    COUNTRY_PATTERNS.put(
        "DE", new PhoneFormat("^\\+49[0-9]{10,11}$", 13, "Germany: +49 XXXX XXXXXXX"));
    COUNTRY_PATTERNS.put(
        "FR", new PhoneFormat("^\\+33[1-9][0-9]{8}$", 12, "France: +33 X XX XX XX XX"));
    COUNTRY_PATTERNS.put("IT", new PhoneFormat("^\\+39[0-9]{9,10}$", 13, "Italy: +39 XXX XXXXXXX"));
    COUNTRY_PATTERNS.put(
        "ES", new PhoneFormat("^\\+34[6-9][0-9]{8}$", 12, "Spain: +34 XXX XXX XXX"));
    COUNTRY_PATTERNS.put(
        "NL", new PhoneFormat("^\\+31[6-9][0-9]{8}$", 12, "Netherlands: +31 X XXXX XXXX"));
    COUNTRY_PATTERNS.put("BE", new PhoneFormat("^\\+32[0-9]{9}$", 12, "Belgium: +32 X XXX XX XX"));
    COUNTRY_PATTERNS.put(
        "CH", new PhoneFormat("^\\+41[0-9]{9}$", 12, "Switzerland: +41 XX XXX XX XX"));
    COUNTRY_PATTERNS.put(
        "AT", new PhoneFormat("^\\+43[0-9]{10,11}$", 13, "Austria: +43 XXXX XXXXXX"));
    ...
    COUNTRY_PATTERNS.put(
        "FI", new PhoneFormat("^\\+358[0-9]{9,10}$", 13, "Finland: +358 XX XXX XXXX"));
    COUNTRY_PATTERNS.put("PL", new PhoneFormat("^\\+48[0-9]{9}$", 12, "Poland: +48 XXX XXX XXX"));
    COUNTRY_PATTERNS.put(
        "CZ", new PhoneFormat("^\\+420[0-9]{9}$", 13, "Czech Republic: +420 XXX XXX XXX"));
    COUNTRY_PATTERNS.put("GR", new PhoneFormat("^\\+30[0-9]{10}$", 13, "Greece: +30 XXX XXX XXXX"));
    COUNTRY_PATTERNS.put(
        "PT", new PhoneFormat("^\\+351[0-9]{9}$", 13, "Portugal: +351 XXX XXX XXX"));
    COUNTRY_PATTERNS.put(
        "IE", new PhoneFormat("^\\+353[0-9]{9}$", 13, "Ireland: +353 XX XXX XXXX"));
  }
  ...
  public static ValidationResult validateMobile(String phone) {
    ...
    if (!isValidE164(phone)) {
      return ValidationResult.invalid("Invalid mobile format. Must be E.164 (e.g., +905551234567)");
    }
  ...
  public static ValidationResult validate(String phone, String countryCode) {
    ...
    if (countryCode != null && !countryCode.isBlank()) {
      ...
      if (format != null && !isValidForCountry(trimmed, countryCode)) {
        return ValidationResult.invalid(
            String.format(
                "Invalid phone format for %s. Expected: %s", countryCode, format.description));
      }
    }
  ...
  public static String getFormatDescription(String countryCode) {
    PhoneFormat format =
        COUNTRY_PATTERNS.get(countryCode != null ? countryCode.toUpperCase() : null);
