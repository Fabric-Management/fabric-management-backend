 ...
 *
 * <p>Immutable representation of quantities with units (kg, meters, pieces, etc.) Ensures
 * type-safety and prevents unit-related calculation errors.
 *
 * <h2>Features:</h2>
 *
 * <ul>
 *   <li>Immutable (thread-safe)
 *   <li>Unit-aware operations
 *   <li>Conversion support
 *   <li>Type-safe arithmetic
 *   <li>Prevents mixing incompatible units
 * </ul>
 *
 * <h2>Usage Example:</h2>
 *
...
public class Unit {
  ...
  public Unit divide(double divisor) {
    return new Unit(
        this.quantity.divide(BigDecimal.valueOf(divisor), 3, RoundingMode.HALF_UP), this.unitCode);
  }

  public Unit divide(BigDecimal divisor) {
    return new Unit(this.quantity.divide(divisor, 3, RoundingMode.HALF_UP), this.unitCode);
  }
  ...
  private void assertSameUnit(Unit other) {
    if (!this.unitCode.equals(other.unitCode)) {
      throw new IllegalArgumentException(
          String.format(
              "Cannot operate on different units: %s and %s", this.unitCode, other.unitCode));
    }
  }
