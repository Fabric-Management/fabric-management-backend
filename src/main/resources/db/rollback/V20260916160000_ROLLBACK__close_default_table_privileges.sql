-- DB-PRIV-2: restore historical SIUD table defaults only.
...
BEGIN
  FOREACH r IN ARRAY ARRAY['fabric_app','fabric_system'] LOOP
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = r) THEN
      RAISE NOTICE '% missing — default-privilege restore skipped (test/CI environment)', r;
      CONTINUE;
    END IF;
