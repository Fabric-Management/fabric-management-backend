...
-- ============================================================================
-- ALTER: Add missing columns if table already exists (from V002)
-- ============================================================================
DO $$
BEGIN
    -- Add department_code if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add department_category_id if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'common_company' 
        AND table_name = 'common_department' 
        AND column_name = 'department_category_id'
    ) THEN
        ALTER TABLE common_company.common_department 
        ADD COLUMN department_category_id UUID REFERENCES common_company.common_department_category(id);
    END IF;
    
    -- Add parent_department_id if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'common_company' 
        AND table_name = 'common_department' 
        ...
        ALTER TABLE common_company.common_department 
        ADD COLUMN parent_department_id UUID REFERENCES common_company.common_department(id);
    END IF;
    
    -- Add is_system_department if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'common_company' 
        AND table_name = 'common_department' 
        AND column_name = 'is_system_department'
    ) THEN
        ALTER TABLE common_company.common_department 
        ADD COLUMN is_system_department BOOLEAN NOT NULL DEFAULT FALSE;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_schema = 'common_company' 
        AND table_name = 'common_department' 
        AND constraint_name = 'uq_department_tenant_code'
    ) THEN
        ALTER TABLE common_company.common_department 
        ADD CONSTRAINT uq_department_tenant_code UNIQUE (tenant_id, department_code);
    END IF;
END $$;
