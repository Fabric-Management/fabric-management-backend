-- =====================================================
-- V010: Fiber Module - Execution Fiber Batch Table
-- =====================================================
-- Tracks physical fiber lots/batches in production
-- Prevents over-issue and race conditions with reservation logic
-- 
-- NOTE: Seed data moved to R__001__fiber_seeds.sql (repeatable migration)
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_exec_fiber_batch_tenant_id ON production.production_execution_fiber_batch(tenant_id);
CREATE INDEX IF NOT EXISTS idx_exec_fiber_batch_fiber_id ON production.production_execution_fiber_batch(fiber_id);
CREATE INDEX IF NOT EXISTS idx_exec_fiber_batch_code ON production.production_execution_fiber_batch(batch_code);
CREATE INDEX IF NOT EXISTS idx_exec_fiber_batch_status ON production.production_execution_fiber_batch(status);
CREATE INDEX IF NOT EXISTS idx_exec_fiber_batch_warehouse_location ON production.production_execution_fiber_batch(warehouse_location);

-- Composite indexes for complex queries
CREATE INDEX IF NOT EXISTS idx_batch_tenant_fiber_status ON production.production_execution_fiber_batch (tenant_id, fiber_id, status);
CREATE INDEX IF NOT EXISTS idx_batch_tenant_loc_status ON production.production_execution_fiber_batch (tenant_id, warehouse_location, status);
